...
// Phase 1: Import terminal utilities for password masking
import { promptForPassword, getSecretInput } from '../utils/terminalUtils';
import { escapeHtml, sanitizeMessage, sanitizeUsername, sanitizeRoomName } from '../utils/sanitization';
            ...
            switch (command) {
                ...
                case '/login':
                    ...
                    if (state.current.loggedIn) {
                        ...
                        xtermRef.current.write('Usage: /login <username>\r\n');
                        writePrompt();
                    } else {
                        // Phase 1: Security - Prompt for password separately without showing it
                        const username = args[0];
                        xtermRef.current.write(`Username: ${username}\r\n`);
                        
                        // Use password masking utility
                        promptForPassword(xtermRef, 'Password: ').then(async (password) => {
                            if (!password) {
                                xtermRef.current.write('Login cancelled.\r\n');
                                writePrompt();
                                return;
                            }
                            ...
                            try {
                                ...
                                if (res.status === 200 && body.token) {
                                    ...
                                    xtermRef.current.write(`Logged in as ${sanitizeUsername(body.user.username)}\r\n`);
